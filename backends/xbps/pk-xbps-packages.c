...
static gchar **
pk_xbps_convert_array (xbps_array_t files)
{
    gchar **files2 = NULL;
    gsize len = (files != NULL ? xbps_array_count (files) : 0);
    gsize i = 0;
    gchar *element = NULL;

    if (len > 0) {
        files2 = g_new (gchar *, len + 1);
        g_assert (files2);
        for (; i < len ; i++) {
            xbps_array_get_cstring (files, i, &element);
            files2[i] = element;
        }
        /* Append NULL */
        files2[len] = NULL;
...
{
    xbps_string_t prep = NULL;
    xbps_array_t files, array, allkeys;
    xbps_object_t obj;
    xbps_dictionary_keysym_t ksym;
    const char *keyname, *file;

    array = allkeys = files = NULL;
    keyname = file = NULL;

    files = xbps_array_create ();
    g_assert (files);
    ...
    for (unsigned int i = 0; i < xbps_array_count(allkeys); i++) {
        ...
        for (unsigned int x = 0; x < xbps_array_count(array); x++) {
            ...
            /* Add file to array */
            xbps_array_add_cstring_nocopy (files, file);

            if (xbps_dictionary_get_cstring_nocopy(obj, "target", &file)) {
                /* Add file to array */
                prep = xbps_string_create_cstring_nocopy(" -> ");
                if (xbps_string_append_cstring (prep, file))
                    xbps_array_add (files, prep);
            }
        }
    }
    ...
    if (len > 0) {
        ...
        for (; i < len; i++) {
            ...
            if (pkgd == NULL) {
                ...
                /* If a remote package was found then set the filter to 'NOT INSTALLED' */
                if (pkgd != NULL)
                                ...
                                gpointer user_data UNUSED)
{
    ...
    if (len > 0) {
        ...
        for (; i < len; i++) {
            ...
            g_autofree xbps_array_t files = NULL;
            gchar **files2 = NULL;
            ...
            /* Get files as xbps_array_t of strings */
            files = pk_xbps_get_pkg_files (dic);
            /* Convert xbps_array_t to (gchar **) */
            files2 = pk_xbps_convert_array (files);

            /* Show data */
            pk_backend_job_files (job, search_ids[i], files2);
            perc += step;
            pk_backend_job_set_percentage (job, perc);
            ...
            len_files = (files != NULL ? xbps_array_count (files) : 0);
            for (int i = 0; i < len_files; i++) {
                obj = xbps_array_get (files, i);
